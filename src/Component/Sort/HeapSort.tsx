...
function HSort(): JSX.Element {
  ...
  // heapSort();
  function heapify(graphNodes: GraphNode[], keyList: number[], idx: number, process: Process[]): number[] {
    ...
    while (tIdx > 0 && parent > -1) {
      ...
      if (graphNodes[tKey[parent]].value < graphNodes[tKey[tIdx]].value) {
        process.push({
  ...
  function heapify2(graphNodes: GraphNode[], keyList: number[], process: Process[]): number[] {
    const tKey = keyList.slice();
    let idx = 0;
    let left = idx * 2 + 1;
    let right = idx * 2 + 2;
    ...
    while (left < tKey.length) {
      if (right < tKey.length) { // 자식이 둘다 존재
        const max = graphNodes[tKey[left]].value > graphNodes[tKey[right]].value ? left : right;
        process.push({
          arr: tKey.slice(), targets: [idx, max], phase: 'compare',
        });
        if (graphNodes[tKey[idx]].value >= graphNodes[tKey[max]].value) {
          break;
        }
        process.push({
        ...
        if (graphNodes[tKey[idx]].value < graphNodes[tKey[left]].value) {
          process.push({
            arr: tKey.slice(), targets: [idx, left], phase: 'compare',
          });
  ...
  export function heapSort(graphNodes: GraphNode[]): Process[] {
    // 부모노드와 비교 해서 자신이 크면 바꿈
    // 자신의 자식노드 중 큰 노드가 자신보다 크면 변경
    // 변경후 다시 자식노드들과 비교해서 큰 노드가 있다면 다시 변경
    // 최상단까지 진행
    let keyList: number[] = [];
    const arrayLength = graphNodes.length;
    ...
    for (let i = 0; i < arrayLength; i += 1) {
      keyList.push(graphNodes[i].key);
      process.push({
        arr: keyList.slice(), targets: [i, i], phase: 'insert',
      });
      keyList = heapify(graphNodes, keyList, i, process).slice();
    }
    ...
    while (keyList.length > 0) {
      ...
      keyList.pop();
      keyList = heapify2(graphNodes, keyList, process).slice();
    }
