...
function MSort(): JSX.Element {
  const [graphBars, setBar] = useState<GraphBar[]>(
    [{
      key: 0, value: 3, color: '#f54141', index: 0, sorted: false, height: 0,
    },
    {
      key: 1, value: 1, color: '#f54141', index: 1, sorted: false, height: 0,
    },
    {
  ...
  const mergeSort = (): void => {
    ...
    const sorted = arrayForSort.slice();
    const merge = (list: number[], left: number, mid: number, right: number): void => {
      let i = left;
      let j = mid + 1;
      let k = left;
      while (i <= mid && j <= right) {
        wholeSortProcess.push({
          arr: arrayForSort.slice(), targets: [i, j], phase: 'compare',
        });
        if (graphBars[list[i]].value <= graphBars[list[j]].value) {
          sorted[k] = list[i];
          k += 1;
          i += 1;
        } else {
      ...
      arrayForSort = sorted.slice();

      wholeSortProcess.push({
        arr: arrayForSort.slice(), targets: [left, right], phase: 'up',
      });
    };

    const divide = (list: number[], left: number, right: number): void => {
      if (left < right) {
        const mid = Math.floor((left + right) / 2);
        divide(arrayForSort, left, mid);
        divide(arrayForSort, mid + 1, right);
        merge(arrayForSort, left, mid, right);
      }
    };
    divide(arrayForSort, 0, arrayLength - 1);
    wholeSortProcess.push({
      arr: arrayForSort.slice(), targets: [arrayLength, arrayLength], phase: 'done',
    });
  };
    ...
    if (playing) {
      ...
      if (depth < processLength && depth > -1) {
        ...
        if (wholeSortProcess[depth].phase === 'done') {
          ...
          for (let i = 0; i < arrayLength; i += 1) {
            if (wholeSortProcess[depth].phase === 'down') {
              for (let j = wholeSortProcess[depth].targets[0]; j <= wholeSortProcess[depth].targets[1]; j += 1) {
                if (temp[wholeSortProcess[depth].arr[i]].index === j) {
                  temp[wholeSortProcess[depth].arr[i]].height = 250;
                }
              }
            ...
            } else if (wholeSortProcess[depth].phase === 'up') {
              for (let j = wholeSortProcess[depth].targets[0]; j <= wholeSortProcess[depth].targets[1]; j += 1) {
                if (temp[wholeSortProcess[depth].arr[i]].index === j) {
              ...
              for (let j = 0; j < arrayLength; j += 1) {
                ...
                temp[wholeSortProcess[depth].arr[j]].color = '#f54141';
              }
            }
          }
          setBar(temp);
  ...
  function makeRandomNumber(): void {
    ...
    for (let j = 0; j < 40; j += 1) {
      const tempBar = {
        key: j, value: Math.floor(Math.random() * 25), color: '#f54141', index: j, sorted: false, height: 0,
      };
      temp.push(tempBar);
    }
    setBar(temp);
